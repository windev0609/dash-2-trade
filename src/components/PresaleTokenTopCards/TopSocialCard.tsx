...
const TableRow: NextPage<ITableRowProps> = ({ row = {}, isM }) => {
  const { value, presale_id, token } = row;
  // const changePercentage = (value.change/ value.count) * 100;
  ...
  return (
    <Link
      href={{
        ...
        query: {
          tokenId: presale_id, // pass the token id
        },
      }}
    >
      <div>
        <div className="h-8 flex space-x-2 items-center cursor-pointer hover:bg-highlight dark:hover:bg-highlight-dark">
          <div className="w-[10%] font-normal leading-[1.125rem] text-text-primary dark:text-text-primary-dark pl-2">
            {token?.logo?
              <RoundedImage
                logo={token?.logo}
          ...
          >
            ...
            <span className={`${textColor}`}>{` ${firstChar}${formatNumber(
              value.change,
              maxFractons
            )}%`}</span>
          </div>
